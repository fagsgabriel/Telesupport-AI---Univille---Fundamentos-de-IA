...
- Busca semântica (RAG com ChromaDB + fallback local)
- Perguntas em linguagem natural (`/ask`) com resposta em texto e áudio (TTS gTTS)
...
Busca semântica nas transcrições indexadas.

### `GET /ask?q=`

Gera resposta com RAG e retorna link de áudio da resposta:

```json
{
  "success": true,
  "data": {
    "question": "Quais reclamações existem?",
    "answer": "...",
    "audioUrl": "/tts/answer-123.mp3",
    "sources": []
  }
}
```
...
Opcional (recomendado para RAG completo): subir ChromaDB local na porta `8000`.
Se o Chroma não estiver disponível, o backend usa índice local automático.
